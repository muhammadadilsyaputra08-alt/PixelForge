...
data class FrameModel(
    ...
    var scale: Float = 1f,
    /** FRAME <-> SPRITE SHEET SYNC: id of the [LayerModel] this frame was cropped from (null
     *  for hand-drawn/blank frames that were never cut from a layer, e.g. Draw Next Frame).
     *  Lets ProjectViewModel.propagateSpriteEdit find "the other copy" of this sprite's pixels
     *  so editing this frame (Frame Bank / Frame Editor / Sprite view) or editing the source
     *  layer (main canvas) keeps both in sync automatically. Null-safe for old saved projects
     *  (missing field decodes to null, same as before this sync feature existed). */
    var sourceLayerId: String? = null
)
...
data class AtlasModel(
    ...
    var height: Int = 0,
    var padding: Int = 1,
    /** BUGFIX: previously not stored anywhere - the Atlas panel's "Sprite Spacing" stepper had
     *  nothing to read its current value FROM, and every callback except the stepper's own
     *  onSpacingChange hardcoded spacing back to 1 when repacking (padding change, power-of-two
     *  toggle, alpha-trim toggle, the "Pack Atlas" button, and every auto-repack after a Frame
     *  Bank change). So any spacing the user picked was silently discarded on the very next
     *  repack - meaning sprites could end up placed edge-to-edge (bounding boxes touching)
     *  despite the user having asked for room between them. Now persisted like padding is. */
    var spacing: Int = 1,
...
data class ProjectModel(
    ...
    // COLOR PALETTE (SESI 9): editable per-project, not a fixed hardcoded list. The Eyedropper
    // writes the picked color straight into whichever slot is currently selected (see
    // ProjectViewModel.applyPixelTool's EYEDROPPER case), and a Hex/RGB dialog can set any slot
    // directly - see ColorPickerRow. Defaults match the original hardcoded palette so existing
    // saved projects (missing this field entirely) still open with the same swatches.
...
fun ProjectModel.deepCopy(): ProjectModel = copy(
    ...
    // Explicit copy for the same cross-thread-mutation reason as everything else here - see
