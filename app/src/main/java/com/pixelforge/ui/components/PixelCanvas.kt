...
fun PixelCanvas(
    ...
    // ZOOM-INVARIANT HANDLE SIZE (bug report, SESI 9): the corner/edge handle dots and reference
    // lines are drawn inside this Canvas at a fixed LOCAL size, then the ancestor Box's
    // graphicsLayer pinch-zoom magnifies this Canvas's entire rendered output (bitmap + handles
    // together) for on-screen zoom - so without this, the handles grow right along with the
    // zoomed-in pixels and cover more and more of the sprite the further in you zoom, instead of
    // the fine-grained precision zooming in is supposed to give you. Passing the ancestor's
    // current zoom factor here lets the handle-drawing code pre-shrink by the same factor so the
    // NET on-screen size stays constant at any zoom level, matching the normal (1x) look.
    ...
    Canvas(
        ...
        referenceBoundary?.let { drawReferenceBox(it, spriteBounds?.top ?: 0, bitmap.width, bitmap.height, canvasScale) }
        selectionBounds?.let { drawBoxHandles(it, selectionHandles(it), bitmap.width, bitmap.height, canvasScale = canvasScale) }
        floatingBox?.let { drawBoxHandles(it, cornerHandles(it), bitmap.width, bitmap.height, cornersOnly = true, canvasScale = canvasScale) }
 ...
 */
private fun DrawScope.drawReferenceBox(ref: ReferenceBoundary, top: Int, bmpW: Int, bmpH: Int, canvasScale: Float) {
    ...
    // Handle dots and this line's own stroke are divided by canvasScale (SESI 9 fix) so their
    // ON-SCREEN size stays constant regardless of pinch-zoom, instead of scaling up right along
    // with the magnified pixels and covering more and more of the actual sprite the further in
    // you zoom - exactly backwards from what zooming in is supposed to help with.
    val dash = androidx.compose.ui.graphics.drawscope.Stroke(
        width = 2f / canvasScale,
    ...
    for (h in referenceHandles(ref, top)) {
        ...
        if (h.id.length == 2) drawCornerSquare(hx, hy, canvasScale) else drawEdgeCircle(hx, hy, canvasScale)
    }
    val paint = android.graphics.Paint().apply {
        color = android.graphics.Color.WHITE
        textSize = 28f / canvasScale
        isAntiAlias = true
        setShadowLayer(3f / canvasScale, 0f, 0f, android.graphics.Color.BLACK)
    }
 ...
 *  boxes: orange squares on corners, cyan circles on edge midpoints (if any). [canvasScale]
 *  (SESI 9): see [drawReferenceBox]'s doc - divides every handle/stroke size by the current
 *  pinch-zoom factor so they render at a constant ON-SCREEN size instead of visually growing
 *  (and covering more of the sprite) the further zoomed in you are. */
private fun DrawScope.drawBoxHandles(box: PfRect, handles: List<HandlePoint>, bmpW: Int, bmpH: Int, cornersOnly: Boolean = false, canvasScale: Float = 1f) {
    val cellW = size.width / bmpW; val cellH = size.height / bmpH
    ...
    drawRect(
        ...
        style = androidx.compose.ui.graphics.drawscope.Stroke(width = 2f / canvasScale)
    )
    for (h in handles) {
        val hx = h.bx * cellW; val hy = h.by * cellH
        if (cornersOnly || h.id.length == 2) drawCornerSquare(hx, hy, canvasScale) else drawEdgeCircle(hx, hy, canvasScale)
    }
}

private fun DrawScope.drawCornerSquare(x: Float, y: Float, canvasScale: Float = 1f) {
    val half = 9f / canvasScale
    drawRect(
    ...
    drawRect(
        ...
        topLeft = Offset(x - half, y - half),
        size = androidx.compose.ui.geometry.Size(half * 2, half * 2),
        style = androidx.compose.ui.graphics.drawscope.Stroke(width = 1.5f / canvasScale)
    )
}

private fun DrawScope.drawEdgeCircle(x: Float, y: Float, canvasScale: Float = 1f) {
    val radius = 11f / canvasScale
    drawCircle(color = Color(0xFF4FC3F7), radius = radius, center = Offset(x, y))
    drawCircle(color = Color.White, radius = radius, center = Offset(x, y), style = androidx.compose.ui.graphics.drawscope.Stroke(width = 1.5f / canvasScale))
}
