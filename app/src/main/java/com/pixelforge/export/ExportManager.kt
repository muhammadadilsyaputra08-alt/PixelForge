...
class ExportManager(private val storage: StorageManager) {
    ...
    fun exportAll(project: ProjectModel, atlasBitmap: Bitmap?, spriteSheetBitmap: Bitmap?): File {
        ...
        workDir.deleteRecursively()
        // Also drop a copy into a findable "Download/PixelForge/<project>" folder (creating it
        // if needed) so the export isn't stuck in app-private storage - see StorageManager.
        val publicPath = storage.exportToPublicDownloads(project.name, zipFile)
        project.exportSettings.lastExportPath = publicPath ?: zipFile.absolutePath
        return zipFile
    }
    ...
    fun backupProject(project: ProjectModel): File {
        ...
        storage.exportToPublicDownloads(project.name, file)
        return file
    }
